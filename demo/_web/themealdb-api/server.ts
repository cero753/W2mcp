/**
 * TheMealDB API MCP Server — GENERATED BY w2mcp from spec-less docs. Do not hand-edit.
 * Source: TheMealDB API API docs.  Auth: none (confidence 0).
 */
...
const BASE_URL = "http://www.themealdb.com";
...
async function call(method: string, path: string, body?: unknown): Promise<string> {
  ...
  if (!res.ok) throw new Error(`TheMealDB API ${res.status}: ${ERROR_MAP[res.status] ?? "error"} — ${text.slice(0, 300)}`);
  return text;
}
...
export function createServer() {
  const server = new McpServer({ name: "themealdb-api", version: "0.1.0" });
...
async function runHttp(port: number) {
  ...
  httpServer.listen(port, () => console.error(`themealdb-api MCP (Streamable HTTP) on :${port}/mcp`));
}
