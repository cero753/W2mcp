 ...
 * Env:
 *   W2MCP_MASTER_KEY   64 hex chars (openssl rand -hex 32)   — credential encryption key
 *   DATABASE_URL        Postgres conn string (Supabase/Neon)  — omit to use a local file store
 *   W2MCP_REGISTRY     path to registry.json {api: serverPath} (default ./registry.json)
...
const registryPath = process.env.W2MCP_REGISTRY || "./registry.json";
