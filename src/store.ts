...
export class FileCredentialStore implements Store {
  ...
  constructor(private path: string, private key: Buffer) {
    if (key.length !== 32) throw new Error("master key must be 32 bytes (W2MCP_MASTER_KEY = 64 hex chars).");
...
export class PgCredentialStore implements Store {
  ...
  static async create(connectionString: string, key: Buffer): Promise<PgCredentialStore> {
    ...
    await pool.query(`
      CREATE TABLE IF NOT EXISTS w2mcp_customers (api_key_hash TEXT PRIMARY KEY, customer_id TEXT NOT NULL);
      CREATE TABLE IF NOT EXISTS w2mcp_credentials (customer_id TEXT NOT NULL, api TEXT NOT NULL, sealed JSONB NOT NULL, PRIMARY KEY (customer_id, api));
    `);
  ...
  async addCustomer(customerId: string, apiKey: string) {
    await this.pool.query(
      `INSERT INTO w2mcp_customers (api_key_hash, customer_id) VALUES ($1,$2) ON CONFLICT (api_key_hash) DO UPDATE SET customer_id=$2`,
      [sha256(apiKey), customerId]);
  }
  async authenticate(apiKey: string): Promise<string | null> {
    if (!apiKey) return null;
    const r = await this.pool.query(`SELECT customer_id FROM w2mcp_customers WHERE api_key_hash=$1`, [sha256(apiKey)]);
    return r.rows[0]?.customer_id ?? null;
  }
  async putCredential(customerId: string, api: string, secret: string) {
    await this.pool.query(
      `INSERT INTO w2mcp_credentials (customer_id, api, sealed) VALUES ($1,$2,$3)
       ON CONFLICT (customer_id, api) DO UPDATE SET sealed=$3`,
      [customerId, api, JSON.stringify(seal(this.key, secret))]);
  }
  async getCredential(customerId: string, api: string): Promise<string | null> {
    const r = await this.pool.query(`SELECT sealed FROM w2mcp_credentials WHERE customer_id=$1 AND api=$2`, [customerId, api]);
...
export async function makeStore(key: Buffer): Promise<Store> {
  ...
  return new FileCredentialStore(process.env.W2MCP_STORE_PATH || "./w2mcp-store.json", key);
}

/** Master key from env (64 hex chars). Generate with: openssl rand -hex 32 */
export function masterKeyFromEnv(): Buffer {
  const hex = process.env.W2MCP_MASTER_KEY;
  if (!hex) throw new Error("set W2MCP_MASTER_KEY to 64 hex chars (openssl rand -hex 32)");
